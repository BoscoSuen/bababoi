...
display_name: <Human Title>
display_name_zh: <Chinese Human Title>
...
steps:
  - step: <int>
    ...
    decision_question_zh: >-
      <Chinese question, required when decision_gate is true>
    depends_on: [<step-number>, ...]   # only earlier steps

manual_review:
  - <prose, one item per line>
manual_review_zh:                 # same length and order as manual_review
  - <Chinese prose, one item per line>
...
final_outputs:
  - id: <output-id>
    ...
    description_zh: <Chinese human-facing description>
```

Chinese workflow documentation is fail-closed: every human-facing field shown
above must have a non-empty Chinese counterpart. `manual_review_zh` must match
`manual_review` item-for-item. `generate_workflow_docs.py --lang zh` never falls
...
Chinese prose may be written as a multi-line folded scalar (`>-`) for
...
| Human-facing Chinese workflow prose is missing, empty, wrongly typed, or list-misaligned | `WF014` |
...
| `WF014` | Required Chinese workflow prose is missing, empty, wrongly typed, or `manual_review_zh` is not aligned |
