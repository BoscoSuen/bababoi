...
Fetch upcoming economic events and data releases using FMP API. The script retrieves raw JSON or text; the assistant filters, assesses impact, and generates the chronological Markdown report.
{: .fs-6 .fw-300 }
...
- Present filtered results as structured Markdown reports with impact analysis (generated by the assistant, not the script)
...
- Legacy note: `https://financialmodelingprep.com/api/v3/economic_calendar` was retired by FMP and must not be used as a fallback.
...
- Rate limit exceeded (429) → Suggest waiting or upgrading FMP tier; re-run after the wait
- Network errors → Check connection and re-run the script
...
> **Responsibility:** The script outputs raw JSON or text. The assistant performs this step using the script's output. No Markdown files are generated automatically; results are displayed in chat and can be saved to `reports/` on request.
...
If the user requested specific filters, note at the top:
