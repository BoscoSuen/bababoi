...
- `risk_profile`: 保持 `conservative|balanced|aggressive`（用于可追溯性）
...
注记（运营方针）：
- 日度运营为 **单次通过**（hints作为概念化输入使用）
- 同日内不使用 `--hints` 重新执行 `auto_detect_candidates.py`

### 9.2 代表命令
```bash
# 单次通过运营: detector先执行一次（不为反映hints而重新执行）
